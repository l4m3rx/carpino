...
#
#      ID   Len                Data
# ID: 0xFFF [8] 0xFF 0xFF 0xFF 0xFF 0xFF 0xFF 0xFF 0xFF
# ID: 0xFFF [3] 0xFF 0xFF 0xFF
...
class CanMsg(object):
    ...
    def parse(self, m):
        ...
        if matchObj:
            try: (frame_id, frame_len, frame_data) = matchObj.groups()
            except: return False  # Broken frame?
            # Check Message ID
            try: self.mid = int(frame_id, 16)
            except: return False  # Broken frame?
            # Check lenght integrity
            try: self.mlen = int(frame_len)
            except: return False  # Braoken frame?

            # Check data integrity
            data = [int(byte, 16) for byte in frame_data.split()]
            self.message = [byte for byte in data if byte >= 0 and byte <= 255]

            # Frame lengh - data lengh check
            if len(self.message) != self.mlen:
                return False
...
class CarStatus(object):
    def __init__(self, debug=False):
        ...
        self.uids = {}
        # Time & Date
        [self.hours, self.minutes, self.seconds] = [1, 0, 0]
        [self.day, self.month, self.year] = [1, 1, 2017]
    ...
    def msg(self, m):
        ...
        if (m.mid == 0x130) and (m.mlen > 1):
            if m.message[0] == 0:
                result.append(self._set('engine', 0))
                result.append(self._set('engine_status', 'Off'))
            elif m.message[0] == 0x40:
                result.append(self._set('engine', 0))
                result.append(self._set('engine_status', 'Off'))
            elif m.message[0] == 0x41:
                result.append(self._set('engine', 1))
                result.append(self._set('engine_status', 'Ready'))
            elif m.message[0] == 0x45:
                result.append(self._set('engine', 2))
                result.append(self._set('engine_status', 'Running'))
            if m.message[1] == 0x40:
                result.append(self._set('key', 1))
                result.append(self._set('key_status', 'True'))
            elif m.message[1] == 0:
                result.append(self._set('key', 0))
                result.append(self._set('key_status', 'False'))
            ...
            result.append(self._set('speed', self.kph))
            if m.message[5] == 50:
                result.append(self._set('handbrake', 1))
            elif m.message[5] == 48:
                result.append(self._set('handbrake', 0))
        elif (m.mid == 0x1D0):
            result.append(self._set('coolant_temp', (m.message[0] - 48)))
            ...
            if m.message[0] == 0x80:
                result.append(self._set('hazard_lights', 0))
                result.append(self._set('left_turnsignal', 0))
                result.append(self._set('right_turnsignal', 0))
            elif (m.message[0] == 0x91):
                result.append(self._set('left_turnsignal', 1))
            elif (m.message[0] == 0xA1):
                result.append(self._set('right_turnsignal', 1))
            elif (m.message[0] == 0xB1):
                result.append(self._set('hazard_lights', 1))
        elif (m.mid == 0x1EE):
            if m.message[0] == 0x00:
                result.append(self._set('full_beam', 0))
                result.append(self._set('left_turnsignal', 0))
                result.append(self._set('right_turnsignal', 0))
                ...
                result.append(self._set('full_beam', 1))
            ...
            if m.message[0]:
                result.append(self._set('door_alarm', 1))
            else:
                result.append(self._set('door_alarm', 0))
        elif (m.mid == 0x202):
            ...
            result.append(self._set('rear_fog', bmsg1[1]))
            result.append(self._set('dip_beam', bmsg1[5]))
            result.append(self._set('main_beam', bmsg1[7]))
            result.append(self._set('front_fog', bmsg1[2]))
            result.append(self._set('brake_light', bmsg1[0]))
            result.append(self._set('interior_light', bmsg2[6]))
        elif (m.mid == 0x1E3) and (m.mlen > 1):
            if m.message[0] == 0xF1:
                result.append(self._set('cabinet_light', 1))
            else:
                result.append(self._set('cabinet_light', 0))
            ...
            result.append(self._set('battery_reset', int(br, 16)))
            result.append(self._set('production_date', (int(pd, 16)/(60*24))))
        elif (m.mid == 0x246) and m.mlen:
            if m.message[0] == 0x3F:
                result.append(self._set('rear_demister', 0))
            elif m.message[0] == 0x7F:
                result.append(self._set('rear_demister', 1))
        elif (m.mid == 0x26E):
            if (m.message[0] == 0) and \
                    (m.message[1] == 0x40) and (m.message[2] == 0x7F):
                result.append(self._set('1key', 1))
                result.append(self._set('1key_status', 'True'))
            ...
            if m.message[0] == 0x00:
                result.append(self._set('wipers', 0))
                result.append(self._set('rear_wiper', 0))
            elif (m.message[0] == 0x01) and (m.mlen > 1):
                result.append(self._set('wipers', 1))
                result.append(self._set('wipers_speed', (m.message[1] - 247)))
            elif (m.message[0] in [0x40, 0x81, 0x10]) and (m.mlen > 1):
                result.append(self._set('rear_wiper', 1))
            elif (m.message[0] == 0x08) and (m.mlen > 1):
                print('[%s] Single wipe' % (self.get_time()))
        elif (m.mid == 0x2CA):
            result.append(self._set('outside_temp', ((m.message[0] - 80) / 2)))
        elif (m.mid == 0x2E6) and (m.mlen == 8):
            result.append(self._set('left_temp', (m.message[7] / 2.0)))
            result.append(self._set('fan_speed', (m.message[5])))
        elif (m.mid == 0x2EA) and (m.mlen == 8):
            result.append(self._set('right_temp', (m.message[7] / 2.0)))
        elif (m.mid == 0x366) and (m.mlen > 2):
            etemp = (m.message[0] - 80) / 2.0
            erange = '0x%s%s' % (hex(m.message[2])[2:], hex(m.message[1])[2:])
            result.append(self._set('external_temp', etemp))
            result.append(self._set('range1', int(erange, 16)/16))
            ...
            result.append(self._set('odometer', int(odm, 16)))
            result.append(self._set('avr_fuel', m.message[3]))
            result.append(self._set('range', int(range0, 16) / 16))
        elif (m.mid == 0x34F):
            if m.message[0] == 0xFE:
                result.append(self._set('handbrake', 1))
            ...
            result.append(self._set('rr_door', bmsg[1]))
            result.append(self._set('rl_door', bmsg[3]))
            result.append(self._set('fr_door', bmsg[5]))
            result.append(self._set('fl_door', bmsg[7]))
            # Bonnet & Boot
            # It appears e87 dosnt have hood status here
            # It is probably populated by another message.
            # result.append(self._set('bonnet_open', bmsg2[1]))
            result.append(self._set('boot_open', bmsg2[7]))
...
#             print '2D6', m.message
#            if m.message[1] == 0xFC:
#                result.append(self._set('aircondition', 0))
#            elif m.message[1] == 0xFD:
#                result.append(self._set('aircondition', 1))
        elif (m.mid == 0x32E) and (m.mlen > 3):
            self.internal_light = m.message[0]
            result.append(
                self._set('internal_temp', ((m.message[3] / 10.0) + 6))
            )
...
#            print self.left_fuel, 'fuel left', self.right_fuel
            ...
            result.append(self._set('avr_fc', afc / 10.0))
            result.append(self._set('avr_speed', akmph / 10.0))
            result.append(
                self._set('avr_fc100', round((235.214 / self.avr_fc), 2)))
        elif (m.mid == 0x380) and (m.mlen == 7):
            self.decode_vin(m.message)
        elif m.mid == 0xF2:
            if m.message[0] == 241:
                result.append(self._set('boot_locked', 0))
            elif m.message[0] > 241:
                result.append(self._set('boot_locked', 1))
#            if m.message[3] == 192:
#                result.append(self._set('boot_open', 0))
#            elif m.message[3] == 193:
#                result.append(self._set('boot_open', 1))
        elif m.mid == 0xEE:
            if m.message[0] == 0x81:
                result.append(self._set('rl_lock', 0))
            elif m.message[0] > 0x81:
                result.append(self._set('rl_lock', 1))
#            if m.message[3] == 0xFC:
#                result.append(self._set('rl_door', 1))
#            elif m.message[3] == 0xFD:
#                result.append(self._set('rl_door', 0))
        elif m.mid == 0xEA:
            if m.message[0] == 0x81:
                result.append(self._set('rr_lock', 0))
            elif m.message[0] > 0x81:
                result.append(self._set('rr_lock', 1))
        elif m.mid == 0xE6:
            if m.message[0] == 0x81:
                result.append(self._set('fr_lock', 0))
            elif m.message[0] > 0x81:
                result.append(self._set('fr_lock', 1))
        elif m.mid == 0xE2:
            if m.message[0] == 0x81:
                result.append(self._set('fl_lock', 0))
            elif m.message[0] > 0x81:
                result.append(self._set('fl_lock', 1))
        elif (m.mid == 0x3B0):
            if m.message[0] == 0xFD:
                result.append(self._set('reverse', 0))
            elif m.message[0] == 0xFE:
                result.append(self._set('reverse', 1))
        elif (m.mid == 0x3B6):
            result.append(self._set('fl_window', m.message[0]))
        elif (m.mid == 0x3B7):
            result.append(self._set('rl_window', m.message[0]))
        elif (m.mid == 0x3B8):
            result.append(self._set('fr_window', m.message[0]))
        elif (m.mid == 0x3B9):
            result.append(self._set('rr_window', m.message[0]))
        elif (m.mid == 0x2F8) and (m.mlen > 6):
            hb, lb = hl_bits(m.message[4])
            month = '0x%s' % hb
            month = int(month, 16)
            result.append(self._set('month', month))
            result.append(self._set('day', m.message[3]))
            result.append(self._set('hours', m.message[0]))
            result.append(self._set('minutes', m.message[1]))
            result.append(self._set('seconds', m.message[2]))
            result.append(self._set('year', binc(m.message[5], m.message[6])))
        elif (m.mid == 0x2B6):
            ...
            result.append(self._set('voltage', cv))
        elif (m.mid == 0x581) and (m.mlen > 3):
            if m.message[3] == 0x28:
                result.append(self._set('seat_belt_alarm', 0))
        elif (m.mid == 0x394) and (m.mlen > 3):
            if m.message[3] == 0x29:
                result.append(self._set('seat_belt_alarm', 1))
        elif (m.mid == 0xA8) and (m.mlen == 8):
            result.append(self._set('brake_power', m.message[7]))
            torque = ((m.message[2] * 256) + m.message[1]) / 32
#            result.append(self._set('torque', torque/100.0)
#            self.calc_kw()
            if m.message[5] == 0xF0:
                result.append(self._set('clutch', 0))
            elif m.message[5] == 0xF1:
                result.append(self._set('clutch', 1))
            if m.message[7] > 20:
                result.append(self._set('brakes', 1))
            else:
                result.append(self._set('brakes', 0))
        elif (m.mid == 0xAA) and (m.mlen > 6):
            rpm = binc(m.message[4], m.message[5]) / 4
            throttle = binc(m.message[2], m.message[3]) / 256
            result.append(self._set('rpm', rpm))
            result.append(self._set('throttle', throttle))
            ...
            if (wheel_position < 360) and (wheel_position > -360):
                result.append(self._set('wheel_position', wheel_position))
        elif (m.mid == 0xCE) and (m.mlen == 8):
            # 1.6 to get in km/h not mph
            self._set('fl_wheel', (binc(m.message[0], m.message[1]) / 24 * 1.6))
            self._set('fr_wheel', (binc(m.message[3], m.message[3]) / 24 * 1.6))
            self._set('rl_wheel', (binc(m.message[4], m.message[5]) / 24 * 1.6))
            self._set('rr_wheel', (binc(m.message[6], m.message[7]) / 24 * 1.6))
        elif (m.mid == 0x2A0):
...
def binc(i, n): return (n * 256) + i
def biny(i, l): return format(i, 'b').zfill(l)
